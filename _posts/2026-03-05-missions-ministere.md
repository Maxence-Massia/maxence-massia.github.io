---
title: "Missions : Support et Infrastructure (Ministère des Armées)"
date: 2026-03-04 10:00:00 +0100
categories: [Entreprise]
tags: [AD,Déploiment_W11]
---

## 🛠️ Contexte Technique
Dans le cadre de mon alternance au sein du **CNAD**, j'ai été chargé du ticketing de l'esemble du plateau Balard (Paris) et des VIP-VOP THA

### 📋 Mes principales missions :
1. **Ajout de droits (Ticketing) :** - Utilisation de l'outil de gestion pour traiter les demandes des utilisateurs.
| [![Documentation Ajout de Droits](/assets/img/entreprise/ticketing-thumb.png)](https://github.com/Maxence-Massia/Portfolio/tree/main/Entreprise/Projets_R%C3%A9aliser/Ajout_Droit_Utilisateur) | **1. Ajout de droits (Ticketing)** <br> Utilisation de l'outil de gestion pour traiter les demandes des utilisateurs. <br> [📂 Voir la documentation technique sur GitHub](https://github.com/Maxence-Massia/Portfolio/tree/main/Entreprise/Projets_R%C3%A9aliser/Ajout_Droit_Utilisateur) |
2. **Déploiement de postes :** - Masterisation et configuration des postes de travail.
   - Installation des logiciels métiers spécifiques.
3. **Administration réseau (si applicable) :**
   - Brassage de prises réseaux.
   - Configuration de VLANs simples.
